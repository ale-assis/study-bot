...
class Client(discord.Client):
    ...
    async def on_ready(self):
        ...
        if not self.synced:  # Checar se os comandos slash foram sincronizados
            ...
            try:
                # Log dos comandos registrados no CommandTree antes da sincronização
                print(f"Comandos registrados no CommandTree: {[command.name for command in self.tree.get_commands(guild=discord.Object(id=Config.ID_DO_SERVIDOR))]}")
...
tribunaldo = Client()
tribunaldo.run(Config.TOKEN) # -> descomenta esta linha e apaga/comenta a parte abaixo pra rodar local (TESTES)
